      ...
      try {
        ...
        if (dateParam) {
          ...
          if (latestDate) {
            ...
            const quizData = await getQuestionsForDate("SignalDecoding", latestDate)
            setQuestions(quizData)
          } else {
    ...
    return (
      <div className="min-h-screen relative">
        <div
          ...
          style={{ backgroundImage: "url('/backgrounds/g3-signal-waves.webp')" }}
        />
        <div className="absolute inset-0 bg-linear-to-r from-[#EFECE7]/40 via-[#E7DFD3]/35 to-[#E2DAD2]/40" />
        <div className="flex items-center justify-center min-h-screen relative z-10">
          <div className="flex flex-col items-center gap-4">
            <Loader2 className="h-12 w-12 animate-spin text-[#184E77]" />
            <p className="text-lg text-[#184E77]">퀴즈를 불러오는 중...</p>
          </div>
        </div>
      </div>
    )
